...
void TickAI(u8 playerId){
	if(g_MatchStatus==MATCH_IN_ACTION || g_MatchStatus == MATCH_BALL_ON_GOALKEEPER){
		...
		// INTERCEPT: If ball carrier is advancing directly toward me, engage regardless of aggression
		if (!amIEffectiveChaser && g_Ball.PossessionPlayerId != NO_VALUE && g_MatchStatus != MATCH_BALL_ON_GOALKEEPER) {
			u8 carrierId = g_Ball.PossessionPlayerId;
			if (g_Players[carrierId].TeamId != playerTeamId) {
				i16 cdx = (i16)g_Players[playerId].X - (i16)g_Ball.X;
				i16 cdy = (i16)g_Players[playerId].Y - (i16)g_Ball.Y;
				u16 dist = (u16)(Math_Abs32(cdx) + Math_Abs32(cdy));
				if (dist < 90) {
					u8 carrierDir = g_Players[carrierId].Direction;
					bool movingToward = false;
					if      (carrierDir == DIRECTION_DOWN       && cdy >  10) movingToward = true;
					else if (carrierDir == DIRECTION_UP         && cdy < -10) movingToward = true;
					else if (carrierDir == DIRECTION_RIGHT      && cdx >  10) movingToward = true;
					else if (carrierDir == DIRECTION_LEFT       && cdx < -10) movingToward = true;
					else if (carrierDir == DIRECTION_DOWN_RIGHT && cdy >  10 && cdx >  10) movingToward = true;
					else if (carrierDir == DIRECTION_DOWN_LEFT  && cdy >  10 && cdx < -10) movingToward = true;
					else if (carrierDir == DIRECTION_UP_RIGHT   && cdy < -10 && cdx >  10) movingToward = true;
					else if (carrierDir == DIRECTION_UP_LEFT    && cdy < -10 && cdx < -10) movingToward = true;
					// Also engage if carrier is stationary and very close (prevents standoff)
					if (movingToward || (carrierDir == DIRECTION_NONE && dist < 50)) amIEffectiveChaser = true;
				}
			}
		}
		...
		if (amIEffectiveChaser) {
			...
			if (g_Ball.PossessionPlayerId == NO_VALUE) {
                ...
                if (predDir != DIRECTION_NONE) {
                     // Only lead if defender is NOT already ahead of carrier in its direction
                     // (prevents runaway: defender ahead of carrier chasing in same direction)
                     i16 pRelY = (i16)g_Players[playerId].Y - (i16)g_Ball.Y;
                     i16 pRelX = (i16)g_Players[playerId].X - (i16)g_Ball.X;
                     bool defAhead = false;
                     if      (predDir == DIRECTION_DOWN       && pRelY >  8) defAhead = true;
                     else if (predDir == DIRECTION_UP         && pRelY < -8) defAhead = true;
                     else if (predDir == DIRECTION_RIGHT      && pRelX >  8) defAhead = true;
                     else if (predDir == DIRECTION_LEFT       && pRelX < -8) defAhead = true;
                     else if (predDir == DIRECTION_DOWN_RIGHT && pRelY >  8 && pRelX >  8) defAhead = true;
                     else if (predDir == DIRECTION_DOWN_LEFT  && pRelY >  8 && pRelX < -8) defAhead = true;
                     else if (predDir == DIRECTION_UP_RIGHT   && pRelY < -8 && pRelX >  8) defAhead = true;
                     else if (predDir == DIRECTION_UP_LEFT    && pRelY < -8 && pRelX < -8) defAhead = true;
                     if (!defAhead) {
                         u8 lead = 32;
                         switch(predDir) {
			...
			CallFnc_VOID_P1(15, TickAIDefensiveSupport, playerId);
		}
	}
}
